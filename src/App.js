...
export default function App() {
  ...
  return (
    <>
      ...
      <Main>
        <Box>
          {/* {isLoading ? <Loading></Loading> : <MovieList movies={movies} />}  */}
          <ListBox>
            {isLoading && <Loading />}
            {!isLoading && !error && (
              <MovieList
          ...
          <section>
            <div className="rating">
              {isWatched ? (
                ...
                <>
                  <StarRating onSetRating={setUserRating}></StarRating>
...
function WatchedSummary({ watched }) {
  ...
  const avgRuntime = average(
    watched.map((movie) => {
      let runtime = movie.Runtime.split(" ");
      if (Number(runtime[0]) > 0) {
        return Number(runtime[0]);
      } else {
        return 120;
      }
    })
    ...
    <li key={movie.imdbID}>
      ...
      <div>
        ...
        <p>
          <span>⏳</span>
          <span>
            {movie.Runtime.split(" ").at(0) > 0 ? movie.Runtime : "120 min"}
          </span>
        </p>
        <button
